...
public class Player1 extends Entity{
	...
	public void render(Graphics g) {
		// 設定圖片
		if (jumping == true || falling == true) g.drawImage(Game.player1Image[Game.player1Image.length-1].getBufferedImage(), x, y, width, height, null);
		else g.drawImage(Game.player1Image[animation].getBufferedImage(), x, y, width, height, null);
	}
	...
	public void update() {
		...
		if (animationDelay >= animation_speed) {
			animation++;
			if (animation >= Game.player1Image.length - 1) {
				animation = 0;
			}
			animationDelay = 0;
		}
